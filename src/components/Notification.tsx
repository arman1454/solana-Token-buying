      ...
      <div className={`flex w-full flex-col`}>
        {
          reversedNotification.map((n, idx) => (
            <Notification key={`${n.message}${idx}`} type={n.type} message={n.message}
              description={n.description} txid={n.txid} onHide={() => {
      ...
      <div className='p-4'>
        <div className="flex items-center">
              ...
              ) : null}
            {
              txid ? (
                <div className='flex flex-row'>
                  <a href={`https://explorer.solana.com/tx/` + txid + `?cluster=${networkConfiguration}`} target="_blank" rel='noreferrer'
          ...
          <div className='ml-4 flex flex-shrink-0 self-start'>
            <button onClick={() => onHide()} className='bg-bkg2 default-transition text-fgd-3 hover:text-fgd-4 inline-flex rounded-md
            focus:outline-none'>
